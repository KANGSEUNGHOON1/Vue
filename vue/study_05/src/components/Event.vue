...
// 9.컨테그트 메뉴
const menuVisivle = ref(false);
const menuX = ref(0);
const menuY = ref(0);
const showMenu = (event) => {
  menuVisivle.value = true;
  menuX.value = event.clientX;
};
const selectOption = (option) => {
  alert(`${option} 선택됨`);
  menuVisivle.value = false;
};
</script>
...
<template>
  <div class="container">
    ...
    <p>입력된 이름: {{ saveName }}</p>
    <!-- 6.더블 클릭 이벤트 -->
    <div class="box" :style="{ backgroundColor: color }" @dblclick="toggleColor">더블 클릭하세요!</div>
    <!-- 7.마우스 위치 추적 -->
    ...
    <ul v-if="menuVisivle" :style="{ top: `${menuY}px`, left: `${menuX}px` }" class="context-menu">
